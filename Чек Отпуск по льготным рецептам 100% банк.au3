...
Local $Way = '\Лог выполнения теста\Чек отпуск по льготным рецептам 100% банк.log'
...
;~ Меняем тип чека на отпуск по льготным рецептам
...
_FileWriteLog(@ScriptDir & $Way, 'Тип чека изменился на отпуск по льготным рецептам')
...
send('{tab}')
Send(100)
_FileWriteLog(@ScriptDir & $Way, 'Задана франшиза 100%')
send('{tab 3}')
